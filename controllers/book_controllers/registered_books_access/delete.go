...
// 	var book bookM.Book
// 	result := configs.DB.Delete(&book, inputID)
...
// 		"search": "Successfully deleted book data",
// 	})
// }
...
func DeleteBook(c echo.Context) error {
	...
	err = configs.DB.Transaction(func(tx *gorm.DB) error {
		...
		result := tx.Model(&bookM.Book{}).Where("id = ?", inputID).Update("deleted_at", time.Now())
		if result.Error != nil {
			return result.Error
		}
		if result.RowsAffected == 0 {
...
func PermanentlyDeleteBook(c echo.Context) error {
	...
	err = configs.DB.Transaction(func(tx *gorm.DB) error {
		...
		if err := tx.Unscoped().Delete(&bookM.Book{}, inputID).Error; err != nil {
			return err
		}
...
func DeleteMultipleBooks(c echo.Context) error {
	...
	err := configs.DB.Transaction(func(tx *gorm.DB) error {
		...
		result := tx.Model(&bookM.Book{}).Where("id IN ?", req.BookIDs).Update("deleted_at", time.Now())
		if result.Error != nil {
			return result.Error
		}
...
func RestoreMultipleBooks(c echo.Context) error {
	...
	result := configs.DB.Model(&bookM.Book{}).Where("id IN ?", req.BookIDs).Update("deleted_at", nil)
	if result.Error != nil {
		return c.JSON(http.StatusInternalServerError, map[string]interface{}{
...
func PermanentlyDeleteMultipleBooks(c echo.Context) error {
	...
	err := configs.DB.Transaction(func(tx *gorm.DB) error {
		...
		result := tx.Unscoped().Delete(&bookM.Book{}, req.BookIDs)
		if result.Error != nil {
			return result.Error
		}
